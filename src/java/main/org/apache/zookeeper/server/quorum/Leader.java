...
public class Leader {
   ...
   // it does not commit ops after committing the reconfig
	...
	void sendAllocatePacket(QuorumPacket qp) {
		synchronized (forwardingFollowers) {
			Set<Long> orderSet=new TreeSet<Long>();
			for (LearnerHandler f : forwardingFollowers) {
				if(f.synced()){
					orderSet.add(f.getSid());
				}
			}
			int i=0;
			...
			for (LearnerHandler f : forwardingFollowers) {
				...
				if(f.isAllocate==true) {//i think members size couldn't no more than 2^16, why zk starter use int?
					...
					allocateInfo=ByteBuffer.allocate(4).putInt(allocateNo).array();
				}
				qp.setData(allocateInfo);
				f.queuePacket(qp);
			}
		}
	}
	...
	public void allocateCommit(long zxid) {
		synchronized(this){
			lastCommitted = zxid;
		}
		QuorumPacket qp = new QuorumPacket(Leader.COMMIT, zxid, null, null);
