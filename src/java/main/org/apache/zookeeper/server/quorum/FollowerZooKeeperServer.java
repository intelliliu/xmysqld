...
public class FollowerZooKeeperServer extends LearnerZooKeeperServer {
	...
	public void allocateCommit(QuorumPacket qp) {
		...
		Request request = pendingTxns.remove();
		byte[] maxId0=((SetDataTxn) request.getTxn()).getData();
		ByteBuffer buffer0= ByteBuffer.allocate(maxId0.length);
		buffer0.put(maxId0);
		buffer0.flip();//need flip
		long maxId=buffer0.getLong();
		byte[] allocateNo=qp.getData();
		ByteBuffer buffer= ByteBuffer.allocate(allocateNo.length);
		buffer.put(allocateNo);
		buffer.flip();//need flip
		int finalNo=0;
		int[] bonus = new int[2];
		switch (allocateNo.length){
			case 4:
				finalNo=buffer.getInt();
				break;
			case 8:
				long temp=buffer.getLong();
				finalNo= (int) (temp & 0xffffffffL);
				bonus[0]= (int) (temp>>32) & 0xff;
				bonus[1]= (int) (temp>>48);
				break;
		}
